...
class ConnectionTests: SQLiteTestCase {
  ...
  func test_init_withInMemory_returnsInMemoryConnection() {
    let db = try! SQLiteConnection(.inMemory)
  ...
  func test_init_withTemporary_returnsTemporaryConnection() {
    let db = try! SQLiteConnection(.temporary)
    XCTAssertEqual("", db.description)
  }

  func test_init_withURI_returnsURIConnection() {
    let db = try! SQLiteConnection(.uri("\(NSTemporaryDirectory())/SQLite.swift Tests.sqlite3"))
  ...
  func test_init_withString_returnsURIConnection() {
    let db = try! SQLiteConnection("\(NSTemporaryDirectory())/SQLite.swift Tests.sqlite3")
    let url = URL(fileURLWithPath: db.description)
    XCTAssertEqual(url.lastPathComponent, "SQLite.swift Tests.sqlite3")
  }
  ...
  func test_readonly_returnsTrueOnReadOnlyConnections() {
    let db = try! SQLiteConnection(readonly: true)
  ...
  func test_lastInsertRowid_doesNotResetAfterError() {
    ...
    XCTAssertThrowsError(
      ...
      if case let SQLiteResult.error(_, code, _) = error {
  ...
  func test_backup_copiesDatabase() throws {
    let target = try SQLiteConnection()
  ...
  func test_transaction_rollsBackTransactionsIfCommitsFail() {
    ...
    do {
      ...
      XCTFail("expected error")
    } catch let SQLiteResult.error(_, code, _) {
      XCTAssertEqual(SQLITE_CONSTRAINT, code)
    } catch {
  ...
  func test_updateHook_setsUpdateHook_withInsert() {
    async { done in
      db.updateHook { operation, db, table, rowid in
        XCTAssertEqual(SQLiteConnection.Operation.insert, operation)
  ...
  func test_updateHook_setsUpdateHook_withUpdate() {
    ...
    async { done in
      db.updateHook { operation, db, table, rowid in
        XCTAssertEqual(SQLiteConnection.Operation.update, operation)
        XCTAssertEqual("main", db)
        XCTAssertEqual("users", table)
        XCTAssertEqual(1, rowid)
        done()
  ...
  func test_updateHook_setsUpdateHook_withDelete() {
    ...
    async { done in
      db.updateHook { operation, db, table, rowid in
        XCTAssertEqual(SQLiteConnection.Operation.delete, operation)
        XCTAssertEqual("main", db)
        XCTAssertEqual("users", table)
        XCTAssertEqual(1, rowid)
        done()
  ...
  func test_concurrent_access_single_connection() {
    ...
    let conn = try! SQLiteConnection("\(NSTemporaryDirectory())/\(UUID().uuidString)")
...
class ResultTests: XCTestCase {
  ...
  func test_init_with_ok_code_returns_nil() {
    XCTAssertNil(SQLiteResult(errorCode: SQLITE_OK, connection: connection, statement: nil) as SQLiteResult?)
  }

  func test_init_with_row_code_returns_nil() {
    XCTAssertNil(SQLiteResult(errorCode: SQLITE_ROW, connection: connection, statement: nil) as SQLiteResult?)
  }

  func test_init_with_done_code_returns_nil() {
    XCTAssertNil(SQLiteResult(errorCode: SQLITE_DONE, connection: connection, statement: nil) as SQLiteResult?)
  }

  func test_init_with_other_code_returns_error() {
    if case let .some(.error(message, code, statement)) =
      SQLiteResult(errorCode: SQLITE_MISUSE, connection: connection, statement: nil)
    {
  ...
  func test_description_contains_error_code() {
    XCTAssertEqual("not an error (code: 21)",
                   SQLiteResult(errorCode: SQLITE_MISUSE, connection: connection, statement: nil)?.description)
  }

  func test_description_contains_statement_and_error_code() {
    let statement = try! SQLiteStatement(connection, "SELECT 1")
    XCTAssertEqual("not an error (SELECT 1) (code: 21)",
                   SQLiteResult(errorCode: SQLITE_MISUSE, connection: connection, statement: statement)?.description)
  }
}
