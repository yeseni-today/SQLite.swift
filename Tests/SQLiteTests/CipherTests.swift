...
class CipherTests: XCTestCase {
    ...
    let db1 = try! SQLiteConnection()
    let db2 = try! SQLiteConnection()
    ...
    func test_keyFailure() {
        ...
        let connA = try! SQLiteConnection(path)
        defer { try! FileManager.default.removeItem(atPath: path) }

        try! connA.key("hello")
        try! connA.run("CREATE TABLE foo (bar TEXT)")

        let connB = try! SQLiteConnection(path, readonly: true)
    ...
    func test_open_db_encrypted_with_sqlcipher() {
        ...
        let conn = try! SQLiteConnection(encryptedFile)
